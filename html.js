...
<html lang="en" with-selection-styled>
  <head>
    ...
    <script>/* dialog */(()=>{const a='*[tabindex]:not([tabindex="-1"]),a[href]:not([tabindex="-1"]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),button:not([disabled])',b=b=>{b.querySelectorAll(a).forEach(a=>{if(a.setAttribute("data-inert",""),a.matches("[tabindex]")){const b=a.getAttribute("tabindex");b&&a.setAttribute("data-original-tabindex",b),a.setAttribute("tabindex",-1)}else a.setAttribute("disabled","")})},c=a=>{a.querySelectorAll("[data-inert]").forEach(a=>{if(a.matches("[tabindex]")){const b=a.getAttribute("data-original-tabindex");a.removeAttribute("tabindex"),null!==b&&a.setAttribute("tabindex",b)}else a.removeAttribute("disabled");a.removeAttribute("data-inert")})},d=b=>c=>{if(!b.contains(c.target)){const d=Array.from(b.querySelectorAll(a));d.unshift(b),b.previousElementSibling===c.target?d[d.length-1].focus():b.nextElementSibling===c.target&&d[0].focus()}},e=a=>b=>{b.target.matches("input[type=text],input[type=email],input[type=number],input[type=password],input[type=search],input[type=tel],input[type=url],textarea")||"Escape"===b.key&&a.close()};window.Dialog=class{constructor(a){this.el=a,this.el.setAttribute("tabindex",0),this.backdropEl=a.closest("[dialog-backdrop]"),b(this.backdropEl),this.trapFocus=d(a),this.checkForEscape=e(this),this.setupClosers()}setupClosers(){const a="[dialog-close]";this.backdropEl.addEventListener("click",b=>{const c=b.target;(c===this.backdropEl||c.matches(a)||c.closest(a))&&this.close()})}open(){c(this.backdropEl),this.originalActiveElement=document.activeElement,this.backdropEl.setAttribute("is-active",""),document.documentElement.setAttribute("is-dialog",""),document.addEventListener("focus",this.trapFocus,!0),document.addEventListener("keyup",this.checkForEscape,!0);const a=this.el.querySelector("[dialog-autofocus]");a?a.focus():this.el.focus()}close(){b(this.backdropEl),this.backdropEl.removeAttribute("is-active",""),document.documentElement.removeAttribute("is-dialog"),document.removeEventListener("focus",this.trapFocus,!0),document.removeEventListener("keyup",this.checkForEscape,!0),this.originalActiveElement&&this.originalActiveElement.focus(),this.el.dispatchEvent(new CustomEvent("dialogClose",{detail:{dialog:this}}))}}})()</script>
